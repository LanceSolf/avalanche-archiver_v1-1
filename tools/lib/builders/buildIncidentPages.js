...
const { getDistance, deg2rad, formatDate } = require('../utils');
...
function buildIncidentPages() {
    ...
    incidents.forEach(inc => {
        ...
        const archiveImagesDir = path.join(incidentsDir, '../incident_images', String(inc.id)); // archive/incident_images/ID
        ...
        if (fs.existsSync(archiveImagesDir)) {
            ...
            imagesHtml = imageFiles.map(file => `
                    <div class="gallery-item">
                        <a href="../incident_images/${inc.id}/${file}" target="_blank">
                            <img src="../incident_images/${inc.id}/${file}" alt="Incident Image">
                        </a>
                    </div>`
            ).join('');
        }
    ...
    const indexHtml = generateIndexPage('Avalanche Incidents', '../../', links, true);
    fs.writeFileSync(path.join(incidentsDir, 'index.html'), indexHtml);

    console.log(`Generated ${count} incident pages.`);
    return count;
}
